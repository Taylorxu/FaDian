...
public class MyTemplateAdapter extends RecyclerView.Adapter {
    ...
    private List<GetWoTempletListByUserAccountResponse.DataBean> datas;
    private IMyTemplate iMyTemplate;

    public MyTemplateAdapter(Context context, List<GetWoTempletListByUserAccountResponse.DataBean> datas) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof MyTemplateAdapter.MyHolder) {
            final GetWoTempletListByUserAccountResponse.DataBean bean = datas.get(position);
            ...
            GetWoTempletListByUserAccountResponse.DataBean.SaveTimeBean bean1 = bean.getSaveTime();
            long num = bean1.getTime();
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            //前面的lSysTime是秒数，先乘1000得到毫秒数，再转为java.util.Date类型
            java.util.Date dt = new Date(num);
